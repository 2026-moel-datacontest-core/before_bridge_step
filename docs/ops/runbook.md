...
- live API smoke: pass
  ...
  - unfair dismissal brief draft: HTTP 200, `rendered_text` 1061자, `missing_fields=12`, `cautions=5`, `evidence_checklist=5`, `cited_articles=3`
...
- Playwright / Cypress / Puppeteer 및 Chrome / Chromium이 없어 실제 DOM click smoke는 미수행
- direct URL guard는 browser runtime의 client-side redirect라 HTTP GET 200만으로 검증하지 않음
- 필요 시 수동 browser rehearsal 또는 Playwright/Chromium 설치 후 click smoke를 별도 실행한다.
...
- 현재 SCN-004 rehearsal 중 SCN-005 문서 타입 추가 작업 혼합
