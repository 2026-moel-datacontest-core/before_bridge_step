...
- 다음 세션에서 broad refactor 없이 **failure mode별 low-risk fix 우선, decomposition은 conditional escalation**로 진행할 수 있게 기준선을 정리
...
- **SCN-004 = citation survival 가설 (미확정).**
  - `근로기준법 제23조`가 retrieval에는 잡혀도 최종 `cited_articles`에서 빠진다는 관찰이 있다.
  - 단, 현재 문서/eval에는 이를 직접 측정한 지표가 없고, task6 스냅샷은 SCN-004를 `Current Corpus Covered`로 기록하고 있다.
  - 대응 전에 **Step 0에서 재현 확인이 선행**되어야 하며, 재현되지 않으면 관련 단계(conditional diversity rerank)는 자동 스킵한다.
  - 대응 축(재현 시): **conservative citation-diversity rerank / coverage-aware context assembly**.
...
- **baseline `117/153` 잔존 partial = answer-side enumeration 문제.**
  - task6 스냅샷 기준 가장 큰 잔존 약점(`KLS-EVAL-010`, `KLS-EVAL-017`, `KLS-EVAL-058`)은 **긴 조문 / 하위 항목 열거형에서 숫자·기간·예외·절차·범위 surface가 불완전**한 패턴이다.
...
1. **Step 0 — SCN-004 재현 확인 (게이트).** 실제 제23조 citation drop이 재현되는지 targeted smoke/eval로 먼저 확인. 재현 안 되면 Step 4 자동 스킵.
2. **Step 1 — 좁은 phrasing normalization.** SCN-005 중심. 기존 `LEGAL_QUERY_HINT_RULES` 확장 수준으로만 스코프 고정. 신규 구조 도입 아님.
3. **Step 2 — answer-side deterministic hardening.** 긴 조문 / 하위 항목 / 숫자·기간·예외·절차·범위 surface 보강. baseline 117/153 개선의 주력.
4. **Step 3 — (조건부) conservative citation-diversity / coverage-aware context assembly.** Step 0 재현된 경우에만 진행.
5. **Step 4 — selective decomposition.** SCN-001 Full 등 composition-heavy 질의에 한정한 escalation path. 기본 경로에는 적용하지 않는다.
...
| Step 1 — phrasing normalization | `citation_grounding_clean = 60/60`, `gold_citation_hit = 60/60`, `JSON/schema failure = 0`, `timeout = 0` | `hit@1` ±1 허용, baseline 117/153 유지 또는 개선 |
| Step 2 — answer-side hardening | `citation_grounding_clean = 60/60`, `gold_citation_hit = 60/60`, `JSON/schema failure = 0`, `timeout = 0` | `hit@1 / hit@3 / hit@5` 동일 유지 (retrieval 변경 아님), baseline 117/153 개선 기대 |
| Step 3 — conditional rerank | `gold_citation_hit = 60/60`, `citation_grounding_clean = 60/60`, `hit@3` 동일 유지 | `hit@1` -1 허용 (단, `hit@3` 유지), baseline 117/153 유지 |
...
- `expected_point_strict_coverage`: current `117/153`, `26 partial`보다 개선
...
즉, 다음 세션 목표는 **"안전성(측정축 1의 grounding/gold 유지)을 유지한 채 baseline coverage(측정축 1의 117/153)와 복합 시나리오 coverage(측정축 2)를 함께 올리는 것"**이다.
...
- next (우선순위 재정렬, 2026-04-15 기준):
  - Step 0 — SCN-004 citation survival 재현 확인 (게이트)
  - Step 1 — 좁은 phrasing normalization (기존 `LEGAL_QUERY_HINT_RULES` 확장)
  - Step 2 — answer-side deterministic hardening (긴 조문 / 하위 항목 / 숫자·기간·예외 surface)
  - Step 3 — (조건부) conservative citation-diversity / coverage-aware rerank
  - Step 4 — selective decomposition (conditional escalation, SCN-001 Full류 한정)
