...
- **Step 4 — selective decomposition**
  - `SCN-001 Full`은 여전히 composition-heavy 질의이지만, Before/After 분리 데모가 안정적이고 decomposition의 regression 검증 비용이 커서 landed path에는 포함하지 않았다.
...
5. **Step 4 — selective decomposition.** 분석만 수행, 현재 landed path에는 미적용.
...
- next (선택적 후속 작업):
  ...
  - `SCN-001 Full` decomposition은 demo 필요성이 다시 생길 때만 별도 세션에서 검토
