...
현재 repo 기준으로 문서 초안 skeleton은 기존 RAG service와 분리된 deterministic layer로 구현한다.
...
- frontend intake form과 UI 연결은 아직 후순위이며, 이번 단계에서는 backend fixture/smoke 기준만 고정한다.
...
- schema skeleton만 추가한 경우:
  - `python -m compileall backend`
  - backend import 확인
- RAG service를 수정하지 않았으면 full 60 eval은 실행하지 않는다.
- document draft service 구현 후에는 별도 smoke를 만든다.
  - 예: `backend/verify/check_document_draft.py`
  - 예: `eval/document_draft_scenarios_v1.json`
