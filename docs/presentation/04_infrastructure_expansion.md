...
업데이트 메모: 2026-04-21 기준 로컬 demo preflight script와 SCN-004-DEMO-FREEZE browser dry-run까지 통과했고, `before` DB source/cache 및 artifact/job 구조 문서화도 진행됐다. 이 문서의 인프라 방향은 유지하되, 최신 로컬 재현성 절차는 `docs/ops/README.md`를 우선한다.
