...
- 발표 직전 backend / frontend를 별도 터미널에서 수동 실행하고 `http://localhost:5090/after`에서 final browser rehearsal을 수행한다.
...
- 기준 URL은 `http://localhost:5090`이며 WSL Playwright Chromium을 우선 사용한다.
- 기존 `http://127.0.0.1:5090`은 Next dev HMR cross-origin warning 이력이 있어 QA 기준 URL로 쓰지 않는다.
...
- Browser dry-run: `http://localhost:5090/after`에서 WSL Playwright Chromium 실제 클릭 검증, `SCN-004-DEMO-FREEZE` fixed path는 `/api/v1/answer` 호출 0회
...
- `http://localhost:5090` 기준 browser runtime happy path 통과
...
- browser QA 기준 URL은 `http://localhost:5090`이다. `http://127.0.0.1:5090`은 QA 기준 URL로 쓰지 않는다.
...
- `npm run dev` 전에 `localhost:5090`에 stale Next server가 떠 있지 않은지 먼저 확인한다.
